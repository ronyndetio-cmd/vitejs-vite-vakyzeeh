...
const API = {
  ...
  async addDog(d) {
    await wait(350);
    const n = { ...d, id: uid(), createdAt: ts(), status: 'available' };
    DB.sDogs([n, ...DB.dogs()]);
    return n;
  },
  async updateDog(id, u) {
    await wait(240);
    const a = DB.dogs().map((d) =>
      d.id === id ? { ...d, ...u, updatedAt: ts() } : d
    );
    DB.sDogs(a);
    return a.find((d) => d.id === id);
  },
  async deleteDog(id) {
    await wait(180);
    DB.sDogs(DB.dogs().filter((d) => d.id !== id));
  },
  ...
  async submitApp(data) {
    const app = {
      ...
      aiLoading: true,
      emailOwnerSent: false,
      emailApplicantSent: false,
    };
...
Respond ONLY with valid JSON (no markdown, no extra text):
{
  ...
  "suggestedQuestions": ["<follow-up question 1>", "<follow-up question 2>"]
      ...
      DB.sApps(
        DB.apps().map((a) =>
          a.id === appId ? { ...a, aiLoading: false, aiError: true } : a
        )
      );
    }
  },
...
— Janet Companion Yorkie Rescue Website`;
    ...
    try {
      ...
      DB.sApps(
        DB.apps().map((a) =>
          a.id === appId ? { ...a, emailOwnerSent: true } : a
        )
      );
    } catch (err) {
      // Fallback: open mailto so the email still goes out
      const sub = encodeURIComponent(
        `🐾 New Adoption Application – ${data.dogName} from ${data.firstName} ${data.lastName}`
      );
      const body = encodeURIComponent(emailBody);
      window.open(
        `mailto:${OWNER_EMAIL}?subject=${sub}&body=${body}`,
        '_blank'
      );
      DB.sApps(
        DB.apps().map((a) =>
          a.id === appId
            ? { ...a, emailOwnerSent: false, emailOwnerFallback: true }
            : a
        )
      );
    }
...
${OWNER_EMAIL}`;
  ...
  async setAppStatus(id, s) {
    await wait(160);
    DB.sApps(DB.apps().map((a) => (a.id === id ? { ...a, status: s } : a)));
  },
  async getGal() {
    await wait(200);
    ...
    if (existing.length === 0) {
      const seeded = SEED_GALLERY.map((item, i) => ({
        ...
        id: `seed_${i}`,
        addedAt: new Date(
          Date.now() - (SEED_GALLERY.length - i) * 86400000
        ).toISOString(),
        seeded: true,
      }));
      DB.sGal(seeded);
  ...
  async addMedia(m) {
    await wait(300);
    const n = { ...m, id: uid(), addedAt: ts() };
    DB.sGal([n, ...DB.gal()]);
    return n;
  },
  async deleteMedia(id) {
    await wait(160);
    DB.sGal(DB.gal().filter((g) => g.id !== id));
  },
};
...
/* ═══════════════════════════════════════════════════════════
   ATOMS
═══════════════════════════════════════════════════════════ */
function Spinner({ size = 20, color = '#c4793a' }: any) {
  return (
    <div
      style={{
        width: size,
      ...
      {toasts.map((t) => (
        <div
          ...
          style={{
            ...
            border: `1px solid ${
              t.type === 'err' ? 'rgba(180,50,50,.25)' : 'rgba(196,121,58,.3)'
            }`,
            boxShadow: '0 8px 32px rgba(44,24,16,.18)',
...
const BV = {
  primary: {
    background: 'linear-gradient(135deg,#b86830,#e8955a)',
    color: '#fff',
    boxShadow: '0 4px 20px rgba(196,121,58,.38)',
  },
  outline: {
    background: 'transparent',
    color: 'var(--terracotta)',
    border: '2px solid rgba(160,82,45,.4)',
  },
  ghost: { background: 'rgba(196,121,58,.08)', color: 'var(--terracotta)' },
  dark: {
    background: 'var(--deep)',
    color: 'var(--ivory)',
    boxShadow: '0 4px 18px rgba(44,24,16,.3)',
  },
  danger: {
    background: '#fff0ee',
    color: '#9a2020',
  ...
  green: {
    background: 'rgba(80,140,80,.1)',
    color: '#2d7a2d',
    border: '1px solid rgba(80,140,80,.3)',
  },
  ivory: {
    background: 'var(--ivory)',
    color: 'var(--deep)',
    border: '2px solid rgba(44,24,16,.15)',
    boxShadow: '0 2px 12px rgba(44,24,16,.12)',
  },
};
  ...
  return (
    ...
    >
      {loading && (
        <Spinner
          size={15}
          color={v === 'primary' || v === 'dark' ? '#fff' : 'var(--warm)'}
        />
      )}
      {children}
    </button>
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      {label && (
        ...
        >
          {label}
          {required && <span style={{ color: 'var(--warm)' }}>*</span>}
        </label>
      )}
      {opts ? (
        <select
        ...
        >
          {opts.map((o) => (
            <option key={o.v} value={o.v} style={{ background: '#fff' }}>
              {o.l}
            </option>
          ))}
        </select>
      ...
      {err && (
        <p style={{ color: '#b03030', fontSize: '.76rem', marginTop: 4 }}>
          {err}
        </p>
      )}
      {hint && (
        <p style={{ color: 'var(--t3)', fontSize: '.76rem', marginTop: 4 }}>
          {hint}
        </p>
      )}
    </div>
...
function Modal({ open, onClose, title, sub, children, wide }: any) {
  useEffect(() => {
  ...
  return (
    ...
    >
      <div
        style={{
          background: wide ? 'var(--ivory2)' : 'var(--ivory)',
          borderRadius: 24,
          width: '100%',
          maxWidth: wide ? 760 : 520,
          maxHeight: '94vh',
          overflowY: 'auto',
          border: '1px solid rgba(196,121,58,.2)',
          boxShadow: '0 32px 80px rgba(44,24,16,.3)',
          animation: 'scaleIn .28s cubic-bezier(.34,1.56,.64,1)',
        }}
      >
        <div
          className="modal-head-pad"
          style={{
            padding: '24px 28px 18px',
            borderBottom: '1px solid rgba(196,121,58,.15)',
            background:
              'linear-gradient(135deg,rgba(196,121,58,.08),transparent)',
            position: 'relative',
          }}
        >
          {title && (
          ...
          {sub && (
            <p style={{ color: 'var(--t2)', fontSize: '.85rem', marginTop: 4 }}>
              {sub}
            </p>
          )}
          <button
        ...
        <div className="modal-body" style={{ padding: '22px 28px 28px' }}>
          {children}
        </div>
      </div>
    </div>
    ...
    <>
      <nav
        style={{
          ...
          transition: 'all .32s',
          boxShadow: sc ? '0 2px 24px rgba(44,24,16,.08)' : 'none',
        }}
      >
        <a
          href="#hero"
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <Btn small onClick={onAdmin} v="ghost">
            Admin
          </Btn>
          <button
            className="show-mob"
            onClick={() => setMob(!mob)}
            style={{
              background: 'rgba(196,121,58,.1)',
              border: '1px solid rgba(196,121,58,.25)',
              borderRadius: 9,
              padding: '7px 8px',
              cursor: 'pointer',
              flexDirection: 'column',
      ...
      {mob && (
        <div className="mob-nav" style={{ zIndex: 300 }}>
          ...
          <Btn
            ...
            v="ghost"
            sx={{ alignSelf: 'flex-start', marginTop: 8 }}
          >
            Admin
          </Btn>
        </div>
      ...
      {[
        ...
        <div
          ...
          style={{
            ...
            animation: `float ${d}s ease-in-out infinite`,
      ...
      <div style={{ position: 'relative', zIndex: 3, maxWidth: 700 }}>
        ...
        <div
          style={{
            ...
            gap: 8,
            marginBottom: 24,
            background: 'rgba(245,176,122,.18)',
            border: '1px solid rgba(245,176,122,.38)',
            borderRadius: 50,
            padding: '6px 18px',
        ...
        >
          <span
            style={{
              background:
                'linear-gradient(135deg,#f5c97a 0%,#e8955a 45%,#f5b07a 100%)',
          ...
          <br />
          <span
            style={{ color: '#fff', textShadow: '0 2px 20px rgba(0,0,0,.3)' }}
          >
            Deserves a
          </span>
          <br />
          <em style={{ fontStyle: 'italic', color: 'rgba(255,255,255,.65)' }}>
            Forever Home
          </em>
        </h1>
        ...
        >
          Janet Companion Yorkie Rescue finds loving homes for Yorkshire
          Terriers — with full health certificates, legal paperwork, and
          official transfer of ownership included with every adoption.
        </p>
        ...
        <div
          style={{
            ...
            gap: 13,
            flexWrap: 'wrap',
            animation: 'fadeUp .55s ease .42s both',
          }}
        >
          <Btn
            onClick={() =>
        ...
        >
          {[
            [available > 0 ? available + '' : '0', 'Available Now'],
            ['$350', 'Adoption Fee'],
            ['100%', 'Vet Checked'],
          ].map(([n, l]) => (
            <div key={l} style={{ position: 'relative' }}>
              ...
              <div
                style={{
                  ...
                  width: 32,
                  height: 2,
                  background: 'linear-gradient(90deg,#f5c97a,transparent)',
                  borderRadius: 2,
                }}
              />
            </div>
      ...
      >
        ...
        <div
          style={{
            ...
            height: 40,
            background:
              'linear-gradient(to bottom,rgba(245,201,122,.6),transparent)',
          }}
        />
      </div>
    </section>
...
/* ═══════════════════════════════════════════════════════════
   DOG CARD
═══════════════════════════════════════════════════════════ */
function DogCard({ dog, onAdopt, admin, onDelete, onEdit, delay = 0 }: any) {
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 20,
        overflow: 'hidden',
        border: `1px solid rgba(196,121,58,${hov ? 0.28 : 0.12})`,
        boxShadow: hov
          ? '0 20px 50px rgba(44,24,16,.18),0 0 0 1px rgba(196,121,58,.15)'
          : '0 4px 24px rgba(44,24,16,.08)',
    ...
    >
      ...
      >
        ...
        <div
          style={{
            ...
            inset: 0,
            background:
              'linear-gradient(to top,rgba(44,24,16,.6) 0%,transparent 50%)',
          }}
        />
        {/* Badges */}
        <div
      ...
      <div style={{ padding: '18px 20px 20px' }}>
        ...
        >
          ...
          {dog.gender && (
            <Tag>
              {dog.gender === 'Female' ? '♀️' : '♂️'} {dog.gender}
            </Tag>
          )}
        ...
        >
          {dog.desc ||
            'A sweet Yorkshire Terrier looking for a loving forever home.'}
        </p>
        <div
          style={{
            display: 'flex',
        ...
        >
          ...
          {dog.createdAt && (
            <span style={{ fontSize: '.69rem', color: 'var(--t3)' }}>
        ...
        {admin ? (
          <div style={{ display: 'flex', gap: 8 }}>
            <Btn v="ghost" small onClick={() => onEdit(dog)} sx={{ flex: 1 }}>
              ✏️ Edit
            </Btn>
            <Btn
              v="danger"
          ...
          >
            💛 Already in a loving home
          </div>
        ) : (
          <Btn full onClick={() => onAdopt(dog)}>
            Adopt {dog.name} 🐾
          </Btn>
        )}
      </div>
  ...
  return (
    <span
      style={{
        ...
        fontSize: '.72rem',
        fontWeight: 600,
        border: '1px solid rgba(196,121,58,.15)',
      }}
    >
      {children}
    </span>
  ...
  return (
    <div
      style={{
        ...
        borderRadius: 20,
        overflow: 'hidden',
        border: '1px solid rgba(196,121,58,.1)',
      }}
    >
      ...
      <div style={{ padding: '18px 20px' }}>
        <div
          className="sk"
          style={{ height: 16, width: '52%', marginBottom: 10 }}
        />
        <div className="sk" style={{ height: 13, marginBottom: 7 }} />
        <div
          className="sk"
          style={{ height: 13, width: '68%', marginBottom: 18 }}
        />
...
/* ═══════════════════════════════════════════════════════════
   DOGS SECTION
═══════════════════════════════════════════════════════════ */
function DogsSection({ dogs, loading, onAdopt, admin, onDelete, onEdit }: any) {
  ...
  return (
    <section
      id="dogs"
      className="sec-pad"
      style={{ padding: '88px 5vw', background: 'var(--ivory2)' }}
    >
      ...
      >
        <div
          style={{
            ...
            height: 1,
            background:
              'linear-gradient(90deg,transparent,rgba(196,121,58,.25))',
          }}
        />
        <span
          style={{
        ...
        <div
          style={{
            ...
            height: 1,
            background:
              'linear-gradient(90deg,rgba(196,121,58,.25),transparent)',
          }}
        />
      </div>
      ...
      <div style={{ textAlign: 'center', marginBottom: 48 }}>
        ...
        >
          Each dog is vet-checked, vaccinated, microchipped & comes with full
          legal ownership paperwork.
        </p>
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 50,
                border: `1.5px solid ${
                  flt === k ? 'var(--warm)' : 'rgba(196,121,58,.22)'
                }`,
                background:
                  flt === k
                    ? 'linear-gradient(135deg,#b86830,#e8955a)'
                    : 'rgba(250,246,240,.8)',
                color: flt === k ? '#fff' : 'var(--t2)',
                cursor: 'pointer',
                fontSize: '.83rem',
                fontWeight: 600,
                transition: 'all .2s',
                boxShadow:
                  flt === k ? '0 3px 14px rgba(196,121,58,.3)' : 'none',
              }}
            >
              {l}
            </button>
      ...
      {loading ? (
        ...
        >
          ...
          <p style={{ fontSize: '1.05rem' }}>
            {dogs.length === 0
              ? 'No Yorkies listed yet — check back soon!'
...
═══════════════════════════════════════════════════════════ */
function AdoptModal({ dog, onClose, onSubmit }: any) {
  ...
  const submit = async () => {
    ...
    setStep(2);
    try {
      const r = await onSubmit({ ...f, dogId: dog.id, dogName: dog.name });
      setRes(r);
      setStep(3);
    } catch {
      setStep(1);
  ...
  return (
    <Modal
      ...
      sub="Complete your application — we'll respond within 24–48 hours"
      onClose={onClose}
    >
      {step === 2 && (
        <div style={{ textAlign: 'center', padding: '32px 0' }}>
          <Spinner size={44} color="var(--warm)" />
          <p style={{ marginTop: 18, color: 'var(--t2)' }}>
            Saving your application & preparing email…
          </p>
        </div>
      )}
      {step === 3 && (
        <div style={{ textAlign: 'center', padding: '16px 0 24px' }}>
          ...
          <p style={{ color: 'var(--t2)', lineHeight: 1.8, marginBottom: 16 }}>
            ...
            <strong style={{ color: 'var(--warm)' }}>{dog?.name}</strong> has
            been received.
          </p>

          {/* Email status cards */}
          <div
            className="ai-sc-grid"
            style={{
              display: 'grid',
          ...
          >
            <div
              style={{
                background: 'rgba(80,140,80,.08)',
                border: '1px solid rgba(80,140,80,.2)',
                borderRadius: 12,
                padding: '12px 14px',
              }}
            >
              ...
              >
                Janet has been emailed at{' '}
                <strong>janethooks85@gmail.com</strong> with your full
                application.
              </p>
            </div>
            <div
          ...
          <div
            style={{
              ...
              border: '1px solid rgba(196,121,58,.18)',
              borderRadius: 14,
              padding: '14px 18px',
              marginBottom: 20,
              textAlign: 'left',
          ...
          >
            ...
            >
              Our AI assistant is analyzing your application now. Janet will see
              a full suitability report in the owner dashboard alongside your
              application.
            </p>
          </div>

          <p
            style={{ color: 'var(--t3)', fontSize: '.83rem', marginBottom: 20 }}
          >
            Application ID:{' '}
            <code
              style={{
          ...
          <p
            style={{ color: 'var(--t3)', fontSize: '.82rem', marginBottom: 22 }}
          >
            We'll be in touch within{' '}
            <strong style={{ color: 'var(--terracotta)' }}>24–48 hours</strong>.
          </p>
          <Btn v="outline" onClick={onClose}>
            Close
          </Btn>
        </div>
      )}
      {step === 1 && (
        <>
          ...
          <Field
            ...
            err={errs.location}
            hint="Helps us understand proximity and local pet regulations."
          />
          <Field
            label="How soon do you want to adopt?"
            id="a6"
            value={f.timeline}
            onChange={s('timeline')}
            opts={[
              { v: 'ASAP', l: 'As soon as possible' },
              { v: '1-2 weeks', l: 'Within 1–2 weeks' },
              { v: '1 month', l: 'Within 1 month' },
              { v: '2-3 months', l: '2–3 months' },
              { v: 'flexible', l: "I'm flexible / just exploring" },
            ]}
          />
          <Field
            ...
            opts={[
              { v: '', l: '— Select —' },
              { v: 'house', l: '🏠 House with yard' },
              { v: 'house-no-yard', l: '🏠 House without yard' },
              { v: 'apartment', l: '🏢 Apartment / Condo' },
              { v: 'other', l: 'Other' },
            ]}
          />
          <Field
            label="Prior Dog Experience"
            id="a8"
            value={f.experience}
            onChange={s('experience')}
            opts={[
              { v: '', l: '— Select —' },
              { v: 'none', l: '🐣 First-time owner' },
              { v: 'some', l: '🐾 Some experience' },
              { v: 'experienced', l: '⭐ Experienced owner' },
              { v: 'yorkie', l: '🏆 Have owned Yorkies before' },
            ]}
          />
          <Field
            label="Tell us about you & your home"
            id="a9"
            value={f.message}
            onChange={s('message')}
            required
            rows={4}
            placeholder="Why do you want a Yorkie? Your home, lifestyle, family…"
            err={errs.message}
          />
          ...
          >
            Application saved to our system & emailed to the owner. Fee
            collected upon approval.
          </p>
        </>
      )}
    </Modal>
  ...
  const docs = [
    {
      ...
      desc: 'A formal agreement covering responsibilities, care standards, and return policy.',
    },
    {
      icon: '🏥',
      title: 'Health Certificate',
      desc: 'Vet-issued document with vaccination history, microchip number, and health clearance.',
    },
    {
      icon: '🔄',
      title: 'Transfer of Ownership',
      desc: 'Official document that legally transfers the dog into your name.',
    },
    {
      icon: '📌',
      title: 'Microchip Registration',
      desc: 'We transfer microchip records to your contact info in national databases.',
    },
    {
      icon: '💉',
      title: 'Vaccination Records',
      desc: 'Complete vaccination history with dates and licensed vet signatures.',
    },
    {
      icon: '📖',
      title: 'Yorkie Care Guide',
      desc: 'First-week guide covering feeding, grooming, training, and bonding.',
    },
  ];
  return (
    <section
      id="about"
      className="sec-pad"
      style={{ padding: '88px 5vw', background: 'var(--ivory)' }}
    >
      {/* Two col */}
      <div
        className="cols-2 tablet-1"
        style={{
      ...
      >
        <div className="sr-l">
          ...
          >
            Janet Companion Yorkie Rescue is a passionate, family-run rescue
            dedicated solely to Yorkshire Terriers. Every dog receives full
            veterinary care, loving socialization, and a thorough screening of
            their future home.
          </p>
          <p
            style={{
          ...
          >
            ...
            with every adoption — including an official transfer of ownership —
            so you and your Yorkie can start your journey with complete
            confidence.
          </p>
          {[
            [
              '🏥',
              'Full Vet Care',
              'Every dog is vaccinated, microchipped & cleared by our licensed vet.',
            ],
            [
              '🏠',
              'Home Screening',
              'We carefully evaluate every applicant to ensure the perfect match.',
            ],
            [
              '💛',
              'Lifetime Support',
              'We stay in touch long after your Yorkie comes home.',
            ],
          ].map(([ico, ttl, dsc]) => (
            <div
              key={ttl}
        ...
        <div className="sr-r" style={{ position: 'relative' }}>
          <div
            style={{
              ...
              boxShadow:
                '0 20px 60px rgba(44,24,16,.15), inset 0 1px 0 rgba(255,255,255,.8)',
            }}
          >
            🐕
          </div>
          ...
          <div
            style={{
              ...
              borderRadius: 20,
              padding: '14px 20px',
              boxShadow: '0 8px 28px rgba(139,58,15,.35)',
              textAlign: 'center',
            }}
          >
            <div
      ...
      <div style={{ textAlign: 'center', marginBottom: 40 }}>
        ...
        <p
          className="sr"
          data-d={130}
          style={{ color: 'var(--t2)', maxWidth: 460, margin: '0 auto' }}
        >
          Full transparency is our promise — every family receives all documents
          they need.
        </p>
      </div>
      <div
      ...
      >
        {docs.map((d, i) => (
          <div
            ...
            onMouseEnter={(e) => {
              e.currentTarget.style.borderColor = 'rgba(196,121,58,.35)';
              e.currentTarget.style.transform = 'translateY(-4px)';
              e.currentTarget.style.boxShadow =
                '0 12px 32px rgba(44,24,16,.12)';
            }}
            onMouseLeave={(e) => {
...
function GalleryImg({ src, alt, style }: any) {
...
function Gallery({ admin, toast }: any) {
  ...
  const prevLb = () => {
    const i = (lbIdx - 1 + items.length) % items.length;
    setLb(items[i]);
    setLbIdx(i);
  };
  const nextLb = () => {
    const i = (lbIdx + 1) % items.length;
    setLb(items[i]);
    setLbIdx(i);
  };
    ...
    >
        ...
        <div>
          ...
          {!loading && items.length > 0 && (
            <p
              className="sr"
              data-d={130}
              style={{ color: 'var(--t3)', fontSize: '.85rem', marginTop: 8 }}
            >
              {items.length} photo{items.length !== 1 ? 's' : ''} · Click any
              image to enlarge
            </p>
          )}
        </div>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          {admin && (
            <>
              ...
              <Btn v="ghost" small onClick={resetGallery}>
                ↺ Reset
              </Btn>
            </>
          )}
      ...
      {loading ? (
        ...
        >
          ...
          <p style={{ fontSize: '1.05rem' }}>
            No gallery items yet.{admin ? ' Add the first one!' : ''}
          </p>
        </div>
      ) : (
        <div className="masonry-grid">
          {items.map((item, i) => (
            <div
              ...
              onMouseEnter={(e) => {
                ...
                e.currentTarget.style.boxShadow =
                  '0 12px 36px rgba(44,24,16,.18)';
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.transform = 'none';
                e.currentTarget.style.borderColor = 'rgba(196,121,58,.12)';
                e.currentTarget.style.boxShadow =
                  '0 2px 12px rgba(44,24,16,.08)';
              }}
            >
              {item.type === 'video' ? (
                <video
              ...
              <div
                style={{
                  ...
                  inset: 0,
                  background:
                    'linear-gradient(to top, rgba(44,24,16,.78) 0%, rgba(44,24,16,.1) 45%, transparent 65%)',
                  display: 'flex',
                  flexDirection: 'column',
              ...
              >
                ...
                {admin && (
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      API.deleteMedia(item.id).then(() => {
                        setItems((g) => g.filter((x) => x.id !== item.id));
                        toast('Removed from gallery.');
                      });
                    }}
                    style={{
                      alignSelf: 'flex-end',
                      background: 'rgba(160,50,50,.85)',
                      border: 'none',
                      borderRadius: '50%',
                      width: 28,
                      height: 28,
              ...
              {item.caption && (
                <div
                  style={{
                    ...
                    right: 0,
                    background:
                      'linear-gradient(to top, rgba(44,24,16,.7), transparent)',
...
function AddMediaModal({ onClose, onSave }: any) {
  ...
  return (
    <Modal
      ...
      sub="Upload or link a photo / video"
      onClose={onClose}
    >
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              border: `1.5px solid ${
                mode === k ? 'var(--warm)' : 'rgba(196,121,58,.22)'
              }`,
              color: mode === k ? 'var(--terracotta)' : 'var(--t3)',
              fontWeight: 600,
              fontSize: '.85rem',
            }}
          >
      ...
      {mode === 'file' ? (
        <>
          ...
          <div
            ...
            style={{
              border: '2px dashed rgba(196,121,58,.25)',
              borderRadius: 14,
              padding: '26px 18px',
              textAlign: 'center',
              cursor: 'pointer',
              marginBottom: 14,
              background: 'rgba(196,121,58,.03)',
              transition: 'border-color .2s',
            }}
            onMouseEnter={(e) =>
              (e.currentTarget.style.borderColor = 'rgba(196,121,58,.5)')
            }
            onMouseLeave={(e) =>
              (e.currentTarget.style.borderColor = 'rgba(196,121,58,.25)')
            }
          >
            {src ? (
              type === 'video' ? (
                <video
                  src={src}
                  style={{ maxHeight: 130, borderRadius: 9 }}
                  controls
                />
              ) : (
                <img
      ...
      <Field
        ...
        value={cap}
        onChange={setCap}
        placeholder="Bella enjoying a sunny afternoon…"
      />
      <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
        <Btn v="ivory" onClick={onClose}>
          Cancel
        </Btn>
        <Btn
          disabled={!src}
          ...
          onClick={async () => {
            if (!src) return;
            setL(true);
            await onSave({ src, type, caption: cap });
            setL(false);
          }}
        >
          Add to Gallery
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Ready to bring a Yorkie home? Click the button below and your email
          app will open with a pre-filled message sent directly to Janet. She
        ...
        >
          📧 Your email goes directly to janethooks85@gmail.com — no forms, no
          waiting.
        </p>
        ...
        >
          <div
            style={{
              ...
              inset: 0,
              background:
                'linear-gradient(135deg,rgba(196,121,58,.08),rgba(196,121,58,.02))',
              pointerEvents: 'none',
            }}
          />
          <div
          ...
          >
            One click opens your email app with everything pre-filled. Janet
            personally reviews every inquiry and responds within 24–48 hours.
          </p>
          <Btn
            onClick={() => {
              const subject = encodeURIComponent(
                '🐾 Adoption Inquiry — Janet Companion Yorkie Rescue'
              );
          ...
          >
            Opens your email app with a pre-filled message directly to
            janethooks85@gmail.com
          </p>
        </div>
        ...
        >
          {[
            ...
            ['💰', 'Adoption Fee', '$350 per dog', null],
            ['⏰', 'Response', 'Within 24–48 hrs', null],
...
/* ═══════════════════════════════════════════════════════════
   ADD / EDIT DOG MODALS
═══════════════════════════════════════════════════════════ */
function AddDogModal({ onClose, onSave }: any) {
  const [f, setF] = useState({
    name: '',
    age: '',
    gender: 'Female',
  ...
  return (
    <Modal
      open
      title="Add New Yorkie"
      sub="Create a new adoption listing"
      onClose={onClose}
      wide
    >
      <div className="cols-2">
      ...
      <Field
        ...
        onChange={s('desc')}
        rows={3}
        placeholder="Personality, temperament, quirks…"
      />
      <div style={{ marginBottom: 14 }}>
        <label
          style={{
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                background: mode === k ? 'rgba(196,121,58,.1)' : 'transparent',
                border: `1.5px solid ${
                  mode === k ? 'var(--warm)' : 'rgba(196,121,58,.2)'
                }`,
                color: mode === k ? 'var(--terracotta)' : 'var(--t3)',
                fontWeight: 600,
                fontSize: '.83rem',
        ...
        {mode === 'file' ? (
          <>
            ...
            <div
              onClick={() => ref.current.click()}
              style={{
                border: '2px dashed rgba(196,121,58,.22)',
                borderRadius: 13,
                padding: '22px',
                textAlign: 'center',
                cursor: 'pointer',
              ...
              onMouseLeave={(e) =>
                (e.currentTarget.style.borderColor = 'rgba(196,121,58,.22)')
              }
            >
              {f.mediaSrc ? (
                f.mediaType === 'video' ? (
                ...
                <>
                  ...
                  <p style={{ color: 'var(--t3)', fontSize: '.87rem' }}>
                    Click to upload photo or video
                  </p>
                </>
              )}
          ...
          <Field
            value={f.mediaSrc}
            onChange={(v) =>
              setF((x) => ({ ...x, mediaSrc: v, mediaType: 'image' }))
            }
            placeholder="https://…"
          />
        )}
      ...
      >
        <input
          ...
          id="aft"
          checked={f.featured}
          onChange={(e) => s('featured')(e.target.checked)}
          style={{ width: 16, height: 16, accentColor: 'var(--warm)' }}
        />
        <label
          htmlFor="aft"
          style={{
...
function EditDogModal({ dog, onClose, onSave }: any) {
  const [f, setF] = useState({
    name: dog.name,
    age: dog.age || '',
...
/* ═══════════════════════════════════════════════════════════
   ADMIN DASHBOARD
═══════════════════════════════════════════════════════════ */
function AdminDash({ onClose, toast }: any) {
  const [tab, setTab] = useState('dogs');
  const [dogs, setDogs] = useState([]);
  const [apps, setApps] = useState([]);
  const [L, setL] = useState(true);
  const [addOpen, setAdd] = useState(false);
  const [editDog, setEdit] = useState(null);
  const [tick, setTick] = useState(0);

  const load = async () => {
    setL(true);
    const [d, a] = await Promise.all([API.getDogs(), API.getApps()]);
    setDogs(d);
    setApps(a);
    setL(false);
  };
  ...
  useEffect(() => {
    const h = apps.some((a) => a.aiLoading);
    if (!h) return;
  ...
  return (
    ...
    >
      ...
      >
        {[
          ['🐕', 'Listings', dogs.length],
          [
            '✅',
            'Available',
            dogs.filter((d) => d.status !== 'adopted').length,
          ],
          ...
          <div
            ...
            style={{
              ...
              borderRadius: 15,
              padding: '18px 16px',
              boxShadow: '0 2px 12px rgba(44,24,16,.06)',
            }}
          >
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 50,
              border: `1.5px solid ${
                tab === k ? 'var(--warm)' : 'rgba(196,121,58,.22)'
              }`,
              background:
                tab === k ? 'linear-gradient(135deg,#b86830,#e8955a)' : '#fff',
              color: tab === k ? '#fff' : 'var(--t2)',
              cursor: 'pointer',
              fontSize: '.86rem',
              fontWeight: 700,
              boxShadow: tab === k ? '0 3px 14px rgba(196,121,58,.28)' : 'none',
              transition: 'all .2s',
            }}
          >
            {l}
      ...
      {L ? (
        <div style={{ textAlign: 'center', padding: 52 }}>
          <Spinner size={36} color="var(--warm)" />
        </div>
      ) : (
        <>
          {tab === 'dogs' && (
            <div>
              ...
              >
                ...
                >
                  {dogs.length} Listing{dogs.length !== 1 ? 's' : ''}
                </h3>
              ...
              {dogs.length === 0 ? (
                ...
                >
                  ...
                  <p>No dogs yet. Add the first one!</p>
                </div>
              ) : (
                <div className="auto-cards">
                  {dogs.map((d, i) => (
          ...
          {tab === 'apps' && (
            <div>
              ...
              >
                ...
                <Btn v="ghost" small onClick={() => setTick((x) => x + 1)}>
                  🔄 Refresh
                </Btn>
              </div>
              {apps.length === 0 ? (
                ...
                >
                  {apps.map((a) => (
                    <div
                      ...
                      style={{
                        background: '#fff',
                        border: `1px solid rgba(196,121,58,${
                          a.status === 'pending' ? 0.25 : 0.1
                        })`,
                        borderRadius: 16,
                        padding: '18px 22px',
                        boxShadow: '0 2px 14px rgba(44,24,16,.06)',
                      }}
                    >
                      <div
                        style={{
                      ...
                      >
                        <div>
                          ...
                          <div
                            style={{ color: 'var(--t3)', fontSize: '.81rem' }}
                          >
                            {a.email} · {a.phone}
                          </div>
                          <div
                          ...
                          >
                            Dog: <strong>{a.dogName}</strong> · {a.location} ·{' '}
                            {a.timeline}
                          </div>
                          {a.living && (
                            <div
                              style={{ color: 'var(--t3)', fontSize: '.78rem' }}
                            >
                              Living: {a.living}
                              {a.experience
                          ...
                          >
                            {new Date(a.submittedAt).toLocaleString()} · ID:{' '}
                          ...
                          {a.status === 'rejected' && (
                            <span
                              style={{
                                ...
                                fontSize: '.72rem',
                                fontWeight: 800,
                                background: 'rgba(160,50,50,.1)',
                                color: '#9a2020',
                                border: '1px solid rgba(160,50,50,.25)',
                              }}
                            >
                              WITHDRAWN
                            </span>
                      ...
                      {a.message && (
                        <p
                          style={{
                            color: 'var(--t2)',
                            fontSize: '.84rem',
                            borderTop: '1px solid rgba(196,121,58,.1)',
                            paddingTop: 9,
                            marginTop: 4,
                          }}
                        >
                      ...
                      >
                        ...
                        <div
                          style={{
                            background:
                              'linear-gradient(135deg,rgba(196,121,58,.12),rgba(196,121,58,.06))',
                            padding: '12px 16px',
                            display: 'flex',
                            alignItems: 'center',
                            justifyContent: 'space-between',
                        ...
                        >
                          ...
                          >
                            ...
                            {a.aiLoading && (
                              <>
                                <Spinner size={13} color="var(--warm)" />
                                <span
                                  style={{
                                    fontSize: '.73rem',
                                    color: 'var(--t3)',
                          ...
                          {a.aiScore != null && (
                            ...
                            >
                              ...
                              {a.aiRecommendation && (
                                <span
                                  style={{
                                    ...
                                    background:
                                      ...
                                      'Highly Recommended'
                                        ? 'rgba(80,140,80,.12)'
                                        : a.aiRecommendation === 'Recommended'
                                        ? 'rgba(100,160,100,.1)'
                                        : a.aiRecommendation === 'Review Needed'
                                        ? 'rgba(196,121,58,.12)'
                                        : 'rgba(160,50,50,.1)',
                                    color:
                                      a.aiRecommendation ===
                                ...
                                >
                                  {a.aiRecommendation === 'Highly Recommended'
                                    ? '⭐ Highly Recommended'
                                    : a.aiRecommendation === 'Recommended'
                                    ? '✅ Recommended'
                                    : a.aiRecommendation === 'Review Needed'
                        ...
                        >
                          {a.aiLoading && (
                            ...
                            >
                              Claude is reviewing this application in the
                              background — check back in a few seconds or click
                              Refresh.
                            </div>
                          )}
                          {a.aiError && !a.aiLoading && (
                            <p style={{ color: '#9a2020', fontSize: '.83rem' }}>
                              AI review unavailable — API may be unreachable.
                            </p>
                          )}
                                  ...
                                  {a.aiStrengths?.length > 0 && (
                                    <div
                                      style={{
                                        ...
                                        border: '1px solid rgba(80,140,80,.18)',
                                        borderRadius: 10,
                                        padding: '10px 12px',
                                      }}
                                    >
                          ...
                          >
                            <span
                              style={{
                                ...
                                padding: '3px 11px',
                                fontWeight: 700,
                                background: a.emailOwnerSent
                                  ? 'rgba(80,140,80,.1)'
                                  : a.emailOwnerFallback
                                  ? 'rgba(196,121,58,.1)'
                                  : 'rgba(160,50,50,.08)',
                                color: a.emailOwnerSent
                                  ? '#2d7a2d'
                                  : a.emailOwnerFallback
                                  ? 'var(--terracotta)'
                                  : '#9a2020',
                            ...
                            <span
                              style={{
                                ...
                                padding: '3px 11px',
                                fontWeight: 700,
                                background: 'rgba(80,140,80,.09)',
                                color: '#2d7a2d',
                                border: '1px solid rgba(80,140,80,.22)',
                              }}
                            >
      ...
      {editDog && (
        <EditDogModal
          ...
          onSave={async (id, u) => {
            const d = await API.updateDog(id, u);
            setDogs((x) => x.map((y) => (y.id === id ? d : y)));
            setEdit(null);
            toast('Updated!');
          }}
        />
...
═══════════════════════════════════════════════════════════ */
function LoginModal({ open, onClose, onSuccess }: any) {
      ...
      <div style={{ textAlign: 'center', marginBottom: 24 }}>
        <div
          style={{
            ...
            borderRadius: '50%',
            background:
              'linear-gradient(135deg,rgba(196,121,58,.15),rgba(196,121,58,.08))',
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        {err && (
          ...
          >
            ...
            <p style={{ color: '#b03030', fontSize: '.8rem', fontWeight: 600 }}>
              {err}
            </p>
          </div>
        )}
...
function Footer() {
  return (
    <footer
      style={{
        background: 'var(--deep)',
        borderTop: '1px solid rgba(196,121,58,.15)',
    ...
    >
      ...
      >
        Dedicated to placing Yorkshire Terriers in loving forever homes.
      </p>
      ...
      >
        {[
          ...
          <a
            ...
            onMouseEnter={(e) => {
              const el = e.currentTarget;
              el.style.color = 'var(--warm2)';
            }}
            onMouseLeave={(e) => {
              const el = e.currentTarget;
              el.style.color = 'rgba(250,246,240,.38)';
            }}
          >
            {lbl}
          </a>
      ...
      <p style={{ fontSize: '.72rem', color: 'rgba(250,246,240,.18)' }}>
        © 2024 Janet Companion Yorkie Rescue · All rights reserved
      </p>
    </footer>
  );
}
